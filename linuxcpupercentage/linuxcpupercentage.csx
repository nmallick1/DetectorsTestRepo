private static string GetQuery(OperationContext<App> cxt)
{
    ...
    $@"LinuxSiteStats
        ...
        | extend CPU = todouble(CpuPercentage), Instance = strcat(substring(Tenant, 0, 6), '_', replace('DedicatedLinuxWebWorkerRole_IN', '', RoleInstance))
        | summarize Average = avg(CPU) by bin(TIMESTAMP, 5m), Tenant, RoleInstance";
}

[AppFilter(AppType = AppType.WebApp, PlatformType = PlatformType.Linux, StackType = StackType.All, InternalOnly = false)]
[Definition(Id = "LinuxCpuPercentage", Name = "CPU Usage", Description = "Average CPU usage consumed by Linux apps", Author = "mikono")]
public async static Task<Response> Run(DataProviders dp, OperationContext<App> cxt, Response res)
{
    ...
    if (veryHighCpuFound)
    {
        var message = String.Format("Very high CPU was detected.");
